...
ll_pp_data = np$load("out/nonlinear_pp_data_temp.npy")

ll_partial = np$load("out/nonlinear_ll_partial_temp.npy")
...
write.table(kl_dist, file = "out/nonlinear_kl_dist.txt", sep = " & ", row.names = F)

models <- c("PPCA-2", paste("PPCA-", 5, sep = "") ,"VAE", "SKIP")
...
# plot discrepancies
fig_title = "img/nonlinear_ppn.pdf"
pdf(fig_title, width = 10, height = 10)
